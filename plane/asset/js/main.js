...
var syfubest = document.getElementById('syfubest');
...
function checkCookie(scr)
{
  var username=getCookie("maxS");
  if (username!="")
  {
	  if(scr > username){
		  console.log(username);
		  setCookie("maxS",scr,365);
		  syfubest.innerHTML='历史最高分：'+scr;
	  }else{
		syfubest.innerHTML='历史最高分：'+username;
	  }
	  
  }
  else 
  {
      setCookie("maxS",0,365);
	  syfubest.innerHTML='历史最高分：'+scr;
  }
}
...
var gameOver = function () {
	...
	{
    	if (localStorage.maxsco)
		{
			if(score>Number(localStorage.maxsco)){
				localStorage.maxsco=score;
			}
		}
		else
		{
		...
		syfubest.innerHTML='历史最高分：'+localStorage.maxsco;
	} else {
